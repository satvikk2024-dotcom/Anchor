...
> Anchor takes a job-posting URL and produces a tailored, factually-grounded application in under 8 minutes of my review time by orchestrating job-posting extraction, multi-source company research, master-resume critique against the JD, tailored material generation with a grounding critic, and stateful tracking through the application lifecycle.
...
2. **Job description fetch:**
   - HTTP request to job URL
   - IF response is HTML with structured JSON-LD → parse directly
   - ELSE → pass raw HTML through `JD Parser Agent` (Ollama, structured output)
   - Result: `{role_title, must_haves[], nice_to_haves[], company_signals[], comp_signals?}`
3. **Parallel company research (sub-workflow, 4 branches):**
   - News fetch (Google News RSS or NewsAPI free tier)
   - GitHub org fetch (if company has public org)
   - Crunchbase lite (if mappable from company name)
   - Company website fetch (homepage + /about + /careers)
   - Each branch caches result to Postgres `companies` table — dedup by domain
4. **Company Synthesizer Agent** consumes all 4 branch outputs:
   - Structured output: `{what_they_do, recent_developments[], tech_signals[], team_size_estimate, culture_signals[], likely_role_context}`
5. Update application row: status `researched`
6. Execute Workflow → Workflow 3
...
**Steps:**
1. Load: application row, parsed JD, company synthesis, master resume entries
2. **Resume Critic Agent** (runs *before* tailoring):
   ...
   - This is the "honest critique" addition — Anchor tells me what's actually weak about my resume for *this specific role* before generating anything
3. **Match Scorer Agent**:
   - Input: critique output + JD + master resume
   - Output: `{score: 0-100, tier: hot/warm/cold, reasoning, red_flags[]}`
4. **Decision branch:**
   - IF score < 50 → Wait node + Slack message: "Match score 42 for [Role] @ [Company]. Reason: [reasoning]. Continue or skip?" Two action buttons that re-invoke the workflow via webhook resume
   - IF score >= 50 → continue
5. **Resume Tailorer Agent:**
   - Input: master resume entries (structured) + JD + critique's suggested angle
   - Output: tailored resume as structured JSON — each line tagged with the master_resume_entry_id it derives from
   - *Hard constraint*: every output line must reference a master entry. The schema enforces this.
6. **Grounding Critic Agent:**
   - Input: tailored resume + master resume
   - Output: `{passes: bool, violations[]}` — checks every claim in tailored output traces to master
   - IF violations → retry tailoring with violation feedback, max 1 retry
7. **Cover Letter Generator Agent:**
   - Input: company synthesis + JD + critique's suggested angle + my master "story bank" (3-5 prepared narratives in Postgres)
   - Output: cover letter draft, max 250 words
8. **Render to PDF** (Code node + headless Chrome via Puppeteer, OR markdown → pandoc for simpler v1)
9. Save PDF to Google Drive (folder: `anchor/applications/{company}_{role}_{date}/`)
10. Create Notion page in Applications database with all artifacts linked
11. Update application row: status `awaiting_review`
12. Slack message: structured card with company, role, match score, link to Notion, link to PDFs
...
| id | uuid PK |
| application_id | fk |
| type | enum: tailored_resume, cover_letter, follow_up_nudge |
...
Nine distinct AI roles. Each does one well-defined job. None of them are general-purpose "summarize this" calls.

| Agent | Primary Input | Output Schema | Key Risk |
|---|---|---|---|
| JD Parser | Raw HTML or text | Structured JD fields | HTML noise → use Trafilatura first |
| Company Synthesizer | 4 source branches | Structured company profile | Empty branches → synthesizer must tolerate partial inputs |
| Resume Critic | Master resume + JD | Strengths/weaknesses/gaps | Generic critiques → prompt for specificity |
| Match Scorer | Critique + JD + master | Score 0-100 + reasoning | Overconfidence → critic-like calibration check |
| Resume Tailorer | Master + JD + angle | Structured resume w/ entry refs | Hallucinated experience → grounding critic |
| Grounding Critic | Tailored + master | Pass/fail + violations | False positives → keep violation reasons specific |
| Cover Letter Gen | Company + JD + story bank | Letter draft, ≤250 words | Generic letters → require concrete reference to company synthesis |
| Follow-up Decision | Application history | Nudge or wait + draft | Annoying nudges → conservative timing defaults |
| Pattern Detector | 4 weeks aggregate data | Insights with evidence | Spurious patterns → require min N for any claim |
